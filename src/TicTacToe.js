...
const reducer = (state, action) => {
    switch (action.type) {
        case SET_WINNER: //action의 이름은 보통 대문자로 하는게 규칙
...
const TicTacToe = () => {
    const [state, dispatch] = useReducer(reducer, initialState); // 세번째 인자는 '지연초기화'인데 거의 안씀!! 이걸로도 충분
    ...
    const onClickTable = useCallback(() => {
        dispatch({ type: SET_WINNER, winner: '⭕' }) // dispatch 안의 중괄호가 action, action이 실행될 때 마다 reducer 함수가 실행
    }, []);
